...
<div class="container-fluid p-0">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h1 class="h3">Pengajuan Surat</h1>
        <div>
            ...
            <div class="card">
                <div class="card-header">
                    <ul class="nav nav-tabs card-header-tabs">
                        <li class="nav-item">
                            <a class="nav-link <?= $status === 'pending' ? 'active' : '' ?>" href="<?= base_url('letter-requests?status=pending') ?>">
                        ...
                        <li class="nav-item">
                            <a class="nav-link <?= $status === 'processing' ? 'active' : '' ?>" href="<?= base_url('letter-requests?status=processing') ?>">
                                <i class="fas fa-spinner me-1"></i> Diproses
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link <?= $status === 'approved' ? 'active' : '' ?>" href="<?= base_url('letter-requests?status=approved') ?>">
                                <i class="fas fa-check me-1"></i> Disetujui
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link <?= $status === 'completed' ? 'active' : '' ?>" href="<?= base_url('letter-requests?status=completed') ?>">
                                <i class="fas fa-check-circle me-1"></i> Selesai
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link <?= $status === 'rejected' ? 'active' : '' ?>" href="<?= base_url('letter-requests?status=rejected') ?>">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const table = $('#letterRequestsTable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '<?= base_url('letter-requests/getDataTable') ?>',
                type: 'GET',
                data: function(d) {
                    d.status = status;
