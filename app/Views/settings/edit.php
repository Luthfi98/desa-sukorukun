        ...
        <div class="col-md-12">
            <div class="card shadow">
                ...
                <div class="card-body">
                    ...
                    <form action="<?= base_url('admin/settings/update/' . $setting['id']) ?>" method="post" enctype="multipart/form-data">
                        <?= csrf_field() ?>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="category" name="category" 
                                        value="<?= old('category', $setting['category']) ?>" required 
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" id="value_type" name="value_type" required>
                                        ...
                                        <option value="boolean" <?= old('value_type', $setting['value_type']) == 'boolean' ? 'selected' : '' ?>>Boolean</option>
                                        <option value="json" <?= old('value_type', $setting['value_type']) == 'json' ? 'selected' : '' ?>>JSON</option>
                                        <option value="file" <?= old('value_type', $setting['value_type']) == 'file' ? 'selected' : '' ?>>File</option>
                                        <option value="image" <?= old('value_type', $setting['value_type']) == 'image' ? 'selected' : '' ?>>Image</option>
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="number" class="form-control" id="order" name="order" 
                                        value="<?= old('order', $setting['order']) ?>" min="0">
                        ...
                        <div class="mb-3" id="value-container">
                            <?php if ($setting['value_type'] == 'image' && !empty($setting['value'])): ?>
                                <div class="mb-3">
                                    ...
                                    <div>
                                        <img src="<?= base_url($setting['value']) ?>" alt="<?= $setting['label'] ?>" class="img-thumbnail" style="max-height: 150px;">
                                    </div>
                                </div>
                            <?php elseif ($setting['value_type'] == 'file' && !empty($setting['value'])): ?>
                                <div class="mb-3">
                                    ...
                                    <div>
                                        <a href="<?= base_url($setting['value']) ?>" target="_blank" class="btn btn-sm btn-info">
                        ...
                        <div class="mb-3 form-check">
                            <input type="checkbox" class="form-check-input" id="is_public" name="is_public" value="1" <?= old('is_public', $setting['is_public']) ? 'checked' : '' ?>>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentValueType = '<?= $setting['value_type'] ?>';
    const currentValue = '<?= addslashes($setting['value']) ?>';
    let editor = null;
    ...
    function updateValueField() {
        ...
        // Show current image/file if type hasn't changed
        if (valueType === 'image' && currentValueType === 'image' && currentValue) {
            html += `
                <div class="mb-3">
                    <label class="form-label">Current Image</label>
                    <div>
            ...
            `;
        } else if (valueType === 'file' && currentValueType === 'file' && currentValue) {
            html += `
                <div class="mb-3">
                    <label class="form-label">Current File</label>
                    <div>
        ...
        switch(valueType) {
            case 'text':
                html += `
                    <label for="value" class="form-label">Value <span class="text-danger">*</span></label>
                    <textarea class="form-control" id="value" name="value">${document.getElementById('value')?.value || currentValue}</textarea>
            ...
            case 'number':
                html += `
                    <label for="value" class="form-label">Value <span class="text-danger">*</span></label>
                    <input type="number" class="form-control" id="value" name="value" value="${document.getElementById('value')?.value || currentValue}" step="any">
            ...
            case 'date':
                html += `
                    <label for="value" class="form-label">Value <span class="text-danger">*</span></label>
                    <input type="date" class="form-control" id="value" name="value" value="${document.getElementById('value')?.value || currentValue}">
                    <small class="text-muted">The date value of this setting</small>
                `;
                break;
            case 'boolean':
                const boolValue = document.getElementById('value')?.value || currentValue;
                html += `
                    <label class="form-label">Value <span class="text-danger">*</span></label>
                    <div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="value" id="value_true" value="true" ${boolValue === 'true' ? 'checked' : ''}>
                            <label class="form-check-label" for="value_true">True</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="value" id="value_false" value="false" ${boolValue === 'false' ? 'checked' : ''}>
            ...
            case 'json':
                html += `
                    <label for="value" class="form-label">Value <span class="text-danger">*</span></label>
                    <textarea class="form-control" id="value" name="value" rows="5">${document.getElementById('value')?.value || currentValue}</textarea>
            ...
            case 'image':
                html += `
                    ...
                    <div class="mb-3">
                        <input type="file" class="form-control" id="value" name="value" accept="image/*">
                    ...
                    ${currentValue ? `
                        <div class="mb-3">
                            <label class="form-label">Current Image</label>
                            <div>
                                <img src="<?= base_url() ?>${currentValue}" alt="Current Image" class="img-thumbnail" style="max-height: 200px;">
                            </div>
                        </div>
                    ` : ''}
                `;
            ...
            default:
                html += `
                    <label for="value" class="form-label">Value <span class="text-danger">*</span></label>
                    <input type="text" class="form-control" id="value" name="value" value="${document.getElementById('value')?.value || currentValue}">
                    <small class="text-muted">The value of this setting</small>
                `;
        }
